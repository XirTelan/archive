...
import { addLogEntry } from "../service/logs";
...
export async function fetchLeaderbord(contestId: string) {
  ...
  if (!contestInfo) {
    await addLogEntry(`Contest Info null`, "error");
    console.error(`Contest Info null`);
    return;
  }
...
export async function getContestInfo(contestId: string, sessionId: string) {
  ...
  if (!$top || !$top?.childNodes) {
    await addLogEntry(`Contest Info: $top.childNodes undefined`, "error");
    console.error(`Contest Info: $top.childNodes undefined`);
    return null;
  }
